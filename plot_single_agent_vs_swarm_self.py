...
"""
Plot average scores for single-agent runs versus swarm runs with self learning.

The script can scan experiment directories or focus on specific runs via CLI
flags. For each experiment it averages agent score series, then aggregates
across all runs in the group. Series are trimmed to the shared length before
plotting so the comparison uses identical timesteps.
"""
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot average scores for single agent vs. swarm self learning experiments."
    )
    parser.add_argument(
        "--single-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        nargs="+",
        help=(
            "Specific swarm self learning experiment directories or JSON files to include; "
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("experiments/single_vs_swarm_self.png"),
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    swarm_files = resolve_experiment_selection(
        args.swarm_dir, args.swarm_experiments, label="swarm self learning"
    )
    ...
    plt.plot(
        x,
        swarm_series,
        label=f"Swarm self learning (n={swarm_scores.n_experiments})",
        color="tab:blue",
        linestyle="--",
        linewidth=2,
    )
    ...
    plt.title("Performance: Single Agent vs. Swarm Self Learning")
